...
import useExceptionVersionData from '../hooks/useExceptionVersionData';

interface Props {
  id: string;
}

function ExceptionVersionsTable({ id }: { id: string }) {
  const { name, versions, rows, isLoading } = useExceptionVersionData(id);
...
function ExceptionVersions({ id }: Props) {
  ...
  return (
    <>
      ...
      >
        <ExceptionVersionsTable id={id} />
      </Drawer>
    </>
  );
}

export default ExceptionVersions;
